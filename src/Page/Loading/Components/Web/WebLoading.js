...
const WebLoading = () => {
  ...
  1. When ${user} enters ‘start analysis’, comprehensively analyze all of ${user}’s answers and generate [Final] content that contains 8 things below ([Identity], [Identity_explanation], [Brand_keywords], [Story_headlines], [Story_contents], [Competency], [Target], [Contents_recommendation]) in Korean as a JSON object.
  ...
  When generating [Final] content, don't ask them to wait, you must generate the result immediately.
  You must generate the [Final] content in JSON format containing "{{", "}}".
        ...
        try {
          const response = await axios.post(
            process.env.REACT_APP_URL + `/${userID}/${chatRoomId}/brandStory`,
            {
              ...
              brandKeywords: result.Brand_Keyword,
